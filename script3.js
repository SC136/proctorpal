...
// Simplified startExamBtn click handler
startExamBtn.addEventListener("click", async () => {
    try {
        ...
        startExamWithCameraBtn.disabled = false;
...
// Simplified start exam with camera button handler
startExamWithCameraBtn.addEventListener("click", async () => {
    try {
        ...
        setTimeout(async () => {
            ...
            // Position monitoring section in bottom right corner
            monitoringSection.style.position = 'fixed';
            monitoringSection.style.bottom = '20px';
            monitoringSection.style.right = '20px';
            monitoringSection.style.width = '200px';
            monitoringSection.style.height = '150px';
            monitoringSection.style.zIndex = '1000';
            monitoringSection.style.background = 'transparent';
            monitoringSection.style.border = '2px solid #ddd';
            monitoringSection.style.borderRadius = '8px';
            monitoringSection.style.overflow = 'hidden';
...
function submitExam() {
    ...
    // Stop monitoring but keep camera feed
    clearInterval(examTimerInterval);
    ...
    // Calculate metrics
    const timeTaken = (30 * 60 - examTimeLeft) / 60;
    const questionsAttempted = document.querySelectorAll('input[type="radio"]:checked').length;
    const mockScore = Math.floor(Math.random() * 41) + 60; // Random score between 60-100

    // Update report content with detailed analysis
    trustMetrics.innerHTML = `
        <div class="analysis-section">
            <h3>Trust Score Analysis</h3>
            <div class="metric-item">
                <p>Final Trust Score: <span class="${trustScore > 80 ? 'success' : 'warning'}">${trustScore}%</span></p>
            </div>
            <div class="metric-item">
                <p>Tab Switches: <span class="${tabSwitches > 2 ? 'warning' : 'success'}">${tabSwitches} times</span></p>
            </div>
            <div class="metric-item">
                <p>Copy/Paste Attempts: <span class="${copyPasteCount > 0 ? 'warning' : 'success'}">${copyPasteCount}</span></p>
            </div>
        </div>
    `;
    ...
    examResults.innerHTML = `
        <div class="analysis-section">
            ...
            <div class="metric-item">
                <p>Final Score: <span class="${mockScore >= 70 ? 'success' : 'warning'}">${mockScore}%</span></p>
            </div>
            <div class="metric-item">
                <p>Time Taken: ${timeTaken.toFixed(2)} minutes</p>
            </div>
            <div class="metric-item">
                <p>Questions Attempted: ${questionsAttempted} of 10</p>
            </div>
        </div>
    `;

    // Keep monitoring section visible but move it to corner
    monitoringSection.style.position = 'fixed';
    monitoringSection.style.bottom = '20px';
    monitoringSection.style.right = '20px';
    monitoringSection.style.width = '200px';
    ...
    // Hide exam section and show report
...
returnToDashboardBtn.addEventListener("click", () => {
    ...
    // Reset monitoring section styles
    monitoringSection.style = '';
    monitoringSection.classList.add('hidden');

    // Hide report section and show dashboard
